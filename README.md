...
- **8th Wall 오픈소스 엔진**(`@8thwall/engine-binary`, SLAM+이미지타깃) + 전역 **three.js 0.150** (글로우 셰이더)
- 기본 타깃: 샘플 문양 `assets/munyang.png` → `image-targets/munyang.json`(+`munyang_luminance.png`)로 컴파일됨
- 결정 근거 → `docs/adr/0002-switch-to-8thwall-slam.md` (0001 대체) · 용어 → `CONTEXT.md`

> ⚠️ **배포 시 `image-targets/` 폴더를 반드시 함께 올리세요** — 엔진이 `image-targets/munyang.json`과 `munyang_luminance.png`를 런타임에 가져옵니다.
> 정렬/스케일이 틀어지면 `index.html` 상단 상수(`PLANE_ASPECT`, `EXTRA_ROT_X`, `SCALE_MULT`) + 하단 `↔/↕`로 보정.
